...
export default {
  ...
  watch: {
    ...
    //     }
    //   },
    //   deep: true, // 深度监听，给对象的所有属性都加上这个监听器，性能开销大
    //   immediate: true // 绑定的时候立即执行
    // },
    'pageOptions.total': {
      handler (newVal) {
        if (newVal > 0) {
    ...
    setTableHeight () {
      this.$nextTick(() => {
        if (this.$refs.chTable) {
          this.tableHeight = window.innerHeight - this.getElementToPageTop(this.$refs.chTable.$el) - this.bottomHeight
        }
      })
    },
    // 获取表格距离顶部的高度
